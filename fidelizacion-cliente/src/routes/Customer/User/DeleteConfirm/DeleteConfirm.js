...
const DeleteConfirm = ({ show, handleClose, data, callback }) => {
  ...
  useEffect(() => {
    ...
    setUserName(data != undefined ? data.username : undefined);
  }, [data]);
  ...
  const deleteUser = (e) => {
    setLoading(true);
    axios
      .delete(process.env.REACT_APP_API_URL + "users/" + id, prepareHeaders())
      .then((rs) => {
        callback();
        setLoading(false);
        handleClose();
    ...
    <>
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Footer>
          ...
          <Button style={{ background: "linear-gradient(135deg, #ff7500 0, #ff6103 12.5%, #ff4c1c 25%, #ff3328 37.5%, #f30d2f 50%, #e10034 62.5%, #d00038 75%, #c1003c 87.5%, #b2003f 100%)" }} variant="danger" onClick={deleteUser}>
          {loading ? (
